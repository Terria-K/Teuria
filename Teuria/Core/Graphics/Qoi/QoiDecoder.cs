...
public static class QoiDecoder 
{
    ...
    {
        if (data.Length < QoiCodec.HeaderSize + QoiCodec.PaddingLength)
        {
        ...
        for (int pxPos = 0; pxPos < pixels.Length; pxPos += channels)
        {
            ...
            else
            {
                ...
                int indexPos2 = ((r & 0xFF) * 3 + (g & 0xFF) * 5 + (b & 0xFF) * 7 + (a & 0xFF) * 11) % QoiCodec.HashTableSize * 4;
                index[indexPos2] = r;
                index[indexPos2 + 1] = g;
                index[indexPos2 + 2] = b;
                index[indexPos2 + 3] = a;
        ...
        int pixelsEnd = data.Length - QoiCodec.PaddingLength;
        for (int padIdx = 0; padIdx < QoiCodec.PaddingLength; padIdx++) 
        {
